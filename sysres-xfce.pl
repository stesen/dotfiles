...
# if ($batt eq "100%") {
#-------------------------------------------------- 
    $statline = sprintf("音乐.%s | 处理器.%2d%%  %s | 内存.%4dM | 温度.%3d°C | 网络.%s | %s", &musicinfo, &cpu_calc, &load_avg, &mem_calc, &temp_read, &net_calc, &timeinfo);
#--------------------------------------------------
# } else {
#     $statline = sprintf("音乐.%s | 处理器.%2d%%  %s | 内存.%4dM | 温度.%3d°C | 网络.%s | 电池.%s | %s", &musicinfo, &cpu_calc, &load_avg, &mem_calc, &temp_read, &net_calc, $batt, &timeinfo);
#-------------------------------------------------- 
#}
print $statline;
exit;
...
sub net_calc {
    my @a = `ifstat -i eth1,wlan0 -q 1 1`;
    #my @a = `ifstat -q 1 1`;
    $a[0] =~ m/(\w+) *(\w+)/;
    my $ifname1 = $1;
    my $ifname2 = $2;
